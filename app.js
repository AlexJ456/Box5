document.addEventListener('DOMContentLoaded', () => {
    ...
    const state = {
        ...
        phaseTime: 4, // Default 4 seconds per phase
        pulseStartTime: null // Added for pulse effect timing
    };

    let wakeLock = null; // Stores the wake lock sentinel
    ...
    function togglePlay() {
        ...
        if (state.isPlaying) {
            ...
            state.pulseStartTime = null; // Reset pulse state
            playTone();
            startInterval();
            animate();
    ...
    function startInterval() {
        ...
        interval = setInterval(() => {
            ...
            if (state.countdown === 1) {
                ...
                state.countdown = state.phaseTime;
                state.pulseStartTime = performance.now(); // Start pulse on phase transition
                playTone();
                 if (state.count === 3 && state.timeLimitReached) { // Check if last phase and time limit reached
    ...
    function animate() {
        ...
        const top = (canvas.height - size) / 2 + 120; // Adjusted top position slightly
        const points = [
        ...
        if (state.pulseStartTime) {
            const pulseElapsed = performance.now() - state.pulseStartTime;
            const pulseDuration = 300; // Pulse duration in milliseconds
            if (pulseElapsed < pulseDuration) {
                const pulseProgress = pulseElapsed / pulseDuration;
                // Use a sin wave for smooth pulse in/out: goes from 1 -> 1.6 -> 1
                const pulseScale = 1 + Math.sin(pulseProgress * Math.PI) * 0.6;
                currentRadius = normalRadius * pulseScale;
            } else {
                state.pulseStartTime = null; // Reset pulse state after duration
            }
        }
        // --- End Pulse Logic ---
        ...
        // Draw the bright red dot with potentially pulsed radius
        ctx.beginPath();
        ctx.arc(currentX, currentY, currentRadius, 0, 2 * Math.PI); // Use currentRadius
    ...
    function render() {
        ...
        if (!state.isPlaying && !state.sessionComplete) {
            ...
            phaseTimeSlider?.addEventListener('input', function() {
                ...
                state.countdown = state.phaseTime; // Update countdown immediately if changed while paused
    ...
    window.addEventListener('resize', () => {
        canvas.width = container.clientWidth;
        canvas.height = container.clientHeight;
        // Re-render if not playing to adjust static elements if needed
        if (!state.isPlaying) {
             // No need to re-render everything, just clear canvas if paused
             const ctx = canvas.getContext('2d');
             ctx.clearRect(0, 0, canvas.width, canvas.height);
        }
    });
